...
while($row = mysqli_fetch_array($covid)){
	...
	$query = mysqli_query($koneksi,"select sum(total_recoverd) as total_recoverd from tb_covid where id_country ='".$row['id_country']."'");
	$row = $query->fetch_array();
	$total_pulih[] = $row['total_recoverd'];
}
?>
<!doctype html>
<html>
...
<body>
	...
	<script>
		var config = {
			...
			data: {
				datasets: [{
					data:<?php echo json_encode($total_pulih); ?>,
					backgroundColor: [
					'rgba(255, 99, 132, 0.2)',
					'rgba(54, 162, 235, 0.2)',
					'rgba(255, 206, 86, 0.2)',
